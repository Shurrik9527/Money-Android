...
import com.moyacs.canary.main.deal.WithdrawalDealActivity;
 ...
 */
public class CapitalFragment extends BaseFragment implements CapitalContract.CapitalView {
    ...
    private CapitalContract.CapitalPresenter mPresenter;
    //提现数据
    ...
    private void getSelectData(int selectTabType) {
        ...
        switch (selectTabType) {
            case 0://交易记录
                if(mPresenter!=null){
                    mPresenter.getTransactionRecordList("0");
                }
                break;
            case 1://挂单
                if(mPresenter!=null){
                    mPresenter.getTransactionRecordList("3");
                }
                break;
            case 2://提现
                if(mPresenter!=null){
    ...
    @Override
    public void setPresenter(CapitalContract.CapitalPresenter presenter) {
...
//        paymentList = result.getContent();
    ...
    public void event(EvenVo evenVo) {
        if (evenVo.getCode() == EvenVo.CHANGE_ORDER_SUCCESS && mPresenter != null) {
            mPresenter.getAccountInfo();
            if (tv1.isSelected()) {
                mPresenter.getTransactionRecordList("0");
            }
